...
The induction is the gateway to MakerSpace: every new member attends it once, and an Exco member or trained runner runs it. This page has two halves. The **overview** below is what the workshop covers, for everyone; the **run checklist** at the top of the page is the tickable run sheet for the runner, saved on the runner's device. Print it if you prefer paper.
